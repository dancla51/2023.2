...
def logiteration(f, k, a, xd, xdd, b, x1x2x3, x4, denom, steptype, showlog):
    if showlog:
        ...
        print(f' {steptype}  {color(a)}a={a:25.20f}{"*" if falpha <= fbeta else " "} {color(alpha)}{n1}={alpha:25.20f}* {color(beta)}{n2}={beta:25.20f}* {color(b)}b={b:25.20f}{"*" if falpha > fbeta else " "}{endcolor}   b-a= {b-a} ')
