...
for (t in levels(thisData$Type)) {
  ...
  numDraws <- as.data.frame(thisMissing)$Rings %>% sum()
  p=thisData %>% filter(Type==t) %>% filter(!as.data.frame(thisMissing)$Rings) %>% select(Rings) 
  hotDeck <- sample(p$Rings, numDraws, replace = TRUE) #With replacement
  impData[thisMissing$Rings]<-hotDeck
}
