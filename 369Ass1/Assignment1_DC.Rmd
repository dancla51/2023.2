...
mean(passed_phase_2)	0.03906250	0.03361345	1.162109
mean(passed_phase_3)	0.01953125	0.01400560	1.394532
**Short Conclusion for Black Saber:**
...
Bug Report example A is a very poor bug report, based on REPREX values. This is because it is not able to be easily replicated by someone who wishes to examine the bug.This fails the reprex philosophy that code *"Should be easy for others to copy + paste + run"*[1]. Someone wishing to run the code will not have my_url saved on their computer so this will cause errors. Also they may not have the 'rvest' library loaded which will cause errors.

Bug Report, by contrast, is much better because it is replicatable and is *"easy for others to copy + paste + run"*[1]. However, it does not meet the reprex expectation that it *"Should be easy for others to digest"*[1]. This is because the output from the link is cluttered, and it is difficult to immediately tell what the issue is, let alone how to fix it.
...
Something I would do differently for next assignment would be to spend more time working on it. For this assignment, I had limited time to work on it due to having my wisdom teeth taken out, so I didn't have time to attempt the bonus question. Next time I would like to attempt any/all bonus questions.
