...
Bug Report example A is a very poor bug report, based on REPREX values. This is because it is not able to be easily replicated by someone who wishes to examine the bug.This fails the reprex philosophy that code *"Should be easy for others to copy + paste + run"*. Someone wishing to run the code will not have my_url saved on their computer so this will cause errors. Also they may not have the 'rvest' library loaded which will cause errors.

Bug Report, by contrast, is much better because it is replicatable and is *"easy for others to copy + paste + run"*. However, it does not meet the reprex expectation that it *"Should be easy for others to digest"*. This is because the output from the link is cluttered, and it is difficult to immediately tell what the issue is, let alone how to fix it.
...
*quotes from https://reprex.tidyverse.org/articles/reprex-dos-and-donts.html*
